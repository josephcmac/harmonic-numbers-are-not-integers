...
*)
section \<open>p-adic norm\<close>
theory Pnorm
...
text \<open>
  We prove the basic properties of the p-adic valuation and the p-adic norm.
\<close>
...
lemma pnorm_pval_zero_D:
  assumes \<open>prime p\<close> \<open>pnorm p x = 1\<close>
...
proof-
  have \<open>pnorm p x = p powr (-pval p x)\<close>
    by (simp add: assms(3) pnorm_simplified)
  also have \<open>\<dots> < 1\<close>
    by (metis (no_types) add.inverse_neutral assms(1) assms(2) less_numeral_extra(1) neg_less_iff_less of_int_0_less_iff of_int_minus of_nat_1 of_nat_less_iff powr_less_mono2_neg powr_one_eq_one prime_gt_1_nat)
  finally show ?thesis
    by simp
qed
...
proof-
  ...
  moreover have \<open>p > 0\<close>
    by (simp add: assms(1) prime_gt_0_nat)
  ultimately have \<open>-pval p x < 0\<close>
    by (metis (mono_tags, hide_lams) less_irrefl_nat less_one linorder_neqE_nat of_int_less_0_iff of_nat_0_less_iff of_nat_1 of_nat_eq_0_iff of_nat_less_iff powr_less_cancel_iff powr_one_eq_one powr_zero_eq_one)
  thus ?thesis
    by simp    
qed
...
proof-
  ...
  moreover have \<open>\<not> (p dvd (snd (quotient_of x)))\<close>
    using \<open>p dvd (fst (quotient_of x))\<close> \<open>prime p\<close> prime_nat_int_transfer pval_comprime 
    by blast
  ultimately show ?thesis 
    by blast
qed
...
next
  ...
  proof-
    ...
    moreover have \<open>fst (quotient_of (x+y)) dvd (m*n' + n*m')\<close>
      ...
      by (metis \<open>\<not> int p dvd n'\<close> \<open>\<not> int p dvd n\<close> dvd_0_right fst_quotient_of_divide mult_eq_0_iff)     
    ultimately show ?thesis
      using dvd_trans 
      by blast 
  qed
